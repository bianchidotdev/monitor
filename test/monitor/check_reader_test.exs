defmodule Monitor.CheckReaderTest do
  ...
  describe ".read!" do
    ...
    test "it returns the checks directly if there are no errors", fixture do
      assert Monitor.CheckReader.read!("test/fixtures/checks/fake_check.yaml") == [
               fixture.test_check
             ]
    end

    test "it raises an error if the check type cannot be found" do
      assert_raise Monitor.Error.CheckValidationError, fn ->
        Monitor.CheckReader.read!("test/fixtures/checks/incomplete_check.yaml")
      end
    end

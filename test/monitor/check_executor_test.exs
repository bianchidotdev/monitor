defmodule Monitor.CheckExecutorTest do
  ...
  test "it returns :ok and the response when successful" do
    expect(Monitor.HTTPClientMock, :request, fn _, _ -> {:ok, %HTTPoison.Response{status_code: 200, body: "200 OK"}} end)
  ...
  test "it returns :ok with the error message and the response when unsuccessful" do
    expect(Monitor.HTTPClientMock, :request, fn _, _ -> {:ok, %HTTPoison.Response{status_code: 500, body: "500 Internal Server Error" }} end)
  ...
  test "it returns :ok with the error message in the case of a timeout" do
    expect(Monitor.HTTPClientMock, :request, fn _, _ -> {:error, %HTTPoison.Error{reason: :timeout, id: nil}} end)
